...
export async function displayProducts(type, key) {
    ...
    if (type === 'all') {
        ...
        addTitleToGallery(gallery, 'Todos los productos');
    ...
    // display all products for a single category
        ...
        if (category != null) {
            ...
            addTitleToGallery(gallery, 'La categoría especificada no existe!');
        }
    }
    // display products given a keyword search
        ...
        if (products.length > 0) {
            addTitleToGallery(gallery, "Resultado de Busqueda");
            addProductsToGallery(gallery, products, products.length, products.length);
        }
        else {
            addTitleToGallery(gallery, "La búsqueda no arrojó resultados.!");
        }
    }
...
export async function displayProductDetails(productId) {
    ...
    if (product) {
        ...
        const gallery = createGallery();
        addTitleToGallery(gallery, "¡Error! Producto no encontrado.");
    }
...
async function getProducts() {  
    try {
        /*
            I'm simulating an API response with a JSON saved. 
        ...
        */
        ...
        console.log('Error al recibir productos de API\n' + err);
    }
}
...
function createProductCard(product) {
    ...
    productCard.innerHTML = `
        ...
        <a href="detalhe-produto.html?id=${product.id}" class="item__anchor">Ver producto</a>
    `;
    return productCard;
}
